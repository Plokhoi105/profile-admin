...
function render() {
  ...
  accountsBody.innerHTML = visible.map((account) => {
    ...
    return `<tr data-id="${account.id}" title="${escapeHtml(account.error || "")}">
      ...
      <td class="email-code-cell">${account.last_email_code ? `<button class="inline-copy copy-email-code" type="button" title="Копировать код из письма">${escapeHtml(account.last_email_code)}</button>` : "—"}</td>
    ...
    </tr>${account.deposit_address ? `<tr class="deposit-subrow" data-id="${account.id}"><td colspan="2"></td><td class="deposit-cell"><div class="deposit-cell-inner"><span class="deposit-chain">${escapeHtml(account.deposit_chain || "BSC")}</span><button class="inline-copy copy-deposit" type="button" title="Копировать адрес депозита">${escapeHtml(account.deposit_address)}</button></div></td><td colspan="7"></td></tr>` : ""}`;
  }).join("");
...
accountsBody.addEventListener("click", async (event) => {
  ...
  if (event.target.closest(".copy-email-code")) {
    const account = state.accounts.find((item) => item.id === id);
    if (account?.last_email_code) return copyText(account.last_email_code, "Код из письма скопирован");
  }
  if (event.target.closest(".copy-deposit")) {
    const account = state.accounts.find((item) => item.id === id);
    if (account?.deposit_address) return copyText(account.deposit_address, "Адрес депозита скопирован");
  }
...
async function saveBybitCookies() {
  ...
  try {
    ...
    btn.disabled = false;
    btn.textContent = "Сохранить и получить адрес";
  }
}
...
async function getDepositAddress() {
  ...
  const chain = $("#deposit-chain").value.trim() || "BSC";
  $("#deposit-error").textContent = "";
  $("#deposit-result").hidden = true;
