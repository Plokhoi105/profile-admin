...
function render() {
  ...
  accountsBody.innerHTML = visible.map((account) => {
    ...
    return `<tr data-id="${account.id}" title="${escapeHtml(account.error || "")}">
      ...
      <td class="deposit-cell">${account.deposit_address ? `<button class="inline-copy copy-deposit" type="button" title="Копировать адрес депозита">${escapeHtml(account.deposit_address)}</button>` : "—"}</td>
