...
function render() {
  ...
  accountsBody.innerHTML = visible.map((account) => {
    ...
    return `<tr data-id="${account.id}" title="${escapeHtml(account.error || "")}">
      ...
      <td class="email-cell"><button class="inline-copy copy-email" type="button" title="Копировать email" aria-label="Копировать email ${escapeHtml(account.email)}">${escapeHtml(account.email)}</button><button class="emails-btn" type="button" title="Входящие письма" aria-label="Входящие письма">✉</button></td>
