...
function render() {
  ...
  accountsBody.innerHTML = visible.map((account) => {
    ...
    </tr>${account.deposit_address ? `<tr class="deposit-subrow" data-id="${account.id}"><td colspan="2"></td><td colspan="8" class="deposit-cell"><span class="deposit-chain">${escapeHtml(account.deposit_chain || "BSC")}</span><button class="inline-copy copy-deposit" type="button" title="Копировать адрес депозита">${escapeHtml(account.deposit_address)}</button></td></tr>` : ""}`;
  }).join("");
